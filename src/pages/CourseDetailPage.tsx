...
const AllVocabularyTab = ({ lessons, level }: { lessons: LessonData[]; level: string }) => {
  ...
  const [activeLesson, setActiveLesson] = useState(0); // 0 = all
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-bold flex items-center gap-2">
          <BookOpen size={18} className="text-primary" /> Vocabulary ({allVocab.length} words)
        </h2>
        <button onClick={() => setShowPinyin(!showPinyin)} className="retro-tag text-primary border-primary cursor-pointer hover:bg-primary/10 transition-colors">
          {showPinyin ? "Hide" : "Show"} Pinyin
        </button>
      </div>
      {/* Lesson filter */}
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-hide">
        <button onClick={() => setActiveLesson(0)} className={`px-3 py-1.5 rounded-full text-xs font-mono whitespace-nowrap transition-colors ${activeLesson === 0 ? "bg-primary text-primary-foreground" : "bg-card brutalist-border hover:bg-muted"}`}>
          All Lessons
        </button>
        {lessons.map(l => (
          <button key={l.id} onClick={() => setActiveLesson(l.id)} className={`px-3 py-1.5 rounded-full text-xs font-mono whitespace-nowrap transition-colors ${activeLesson === l.id ? "bg-primary text-primary-foreground" : "bg-card brutalist-border hover:bg-muted"}`}>
            L{l.id}: {l.chinese}
          </button>
        ))}
      </div>
          ...
          <div className="brutalist-card rounded-xl bg-card overflow-hidden">
            {lesson.vocab.map((word, i) => (
              <div key={i} className={`flex items-center p-4 ${i !== lesson.vocab.length - 1 ? "border-b-2 border-border" : ""} hover:bg-muted/50 transition-colors`}>
                ...
                <button onClick={() => speakChinese(word.chinese)} className="ml-3 w-8 h-8 rounded-full bg-muted flex items-center justify-center hover:bg-foreground/10 transition-colors shrink-0">
                  <Volume2 size={14} />
                </button>
              </div>
            ))}
          ...
          <div className="space-y-3">
            {points.map((gp, i) => (
              <div key={i} className="brutalist-card rounded-xl bg-card p-5">
                ...
                <div className="p-2 bg-muted rounded-lg font-mono text-sm mb-2">
                  Pattern: <span className="font-bold">{gp.pattern}</span>
                </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
          {lesson.vocab.map((word, i) => (
            <div key={i} className="brutalist-card rounded-xl bg-card p-4 text-center">
              ...
              <button onClick={() => speakChinese(word.chinese)} className="mt-2 w-7 h-7 rounded-full bg-muted flex items-center justify-center hover:bg-foreground/10 transition-colors mx-auto">
                <Volume2 size={12} />
              </button>
            </div>
          ))}
  ...
  return (
    <div>
      ...
      <div className="brutalist-card rounded-xl bg-card p-8 text-center">
        ...
        {selected && (
          <button onClick={next} className="mt-6 bg-primary text-primary-foreground px-6 py-2 rounded-lg font-mono text-sm font-bold hover:opacity-90 transition-opacity">Next →</button>
        )}
      </div>
    </div>
  );
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${
                activeTab === tab.id ? "bg-primary text-primary-foreground" : "bg-card brutalist-border hover:bg-muted"
              }`}
            >
              <Icon size={14} />
              {tab.label}
