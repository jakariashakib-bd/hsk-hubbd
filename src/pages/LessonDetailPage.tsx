...
import { ArrowLeft, Volume2, BookOpen, Headphones, Code2, Pen, Globe, Brain, Settings2, CheckCircle2, RotateCcw, ChevronLeft, ChevronRight, Shuffle, Eye, EyeOff, Loader2 } from "lucide-react";
import { useCompletedLessons } from "@/hooks/useCompletedLessons";
import Breadcrumb from "@/components/Breadcrumb";
import { useHskLessons } from "@/hooks/useHskData";
import type { VocabWord } from "@/data/hsk1-lessons";
...
const VocabularyTab = ({ vocab, showPinyin, setShowPinyin }: { vocab: VocabWord[]; showPinyin: boolean; setShowPinyin: (v: boolean) => void }) => (
  <div>
    <div className="flex items-center justify-between mb-4">
      <h2 className="font-bold flex items-center gap-2"><BookOpen size={18} className="text-primary" /> Vocabulary ({vocab.length})</h2>
...
const ReadingTab = ({ vocab, lessonTitle }: { vocab: VocabWord[]; lessonTitle: string }) => {
  const sentences = useMemo(() => {
    ...
    for (let i = 0; i < vocab.length - 1; i += 2) {
      ...
      s.push({ chinese: `${a.chinese}${b.chinese}。`, pinyin: `${a.pinyin} ${b.pinyin}.`, english: `${a.english} / ${b.english}.` });
    }
    return s;
  }, [vocab]);
    ...
    <div>
      ...
      <div className="brutalist-card rounded-xl bg-card p-8 text-center">
        ...
        <div className="flex items-center justify-center gap-3 mt-6">
          ...
          <button onClick={() => { setCurrentIndex(Math.floor(Math.random() * vocab.length)); setShowAnswer(false); }} className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center hover:bg-foreground/10 transition-colors"><Shuffle size={18} /></button>
          <button onClick={() => { setCurrentIndex(Math.min(vocab.length - 1, currentIndex + 1)); setShowAnswer(false); }} disabled={currentIndex === vocab.length - 1} className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center hover:bg-foreground/10 transition-colors disabled:opacity-30"><ChevronRight size={18} /></button>
        </div>
      </div>
    </div>
  );
...
const GrammarTab = ({ vocab }: { vocab: VocabWord[] }) => {
  const grammarPoints = useMemo(() => {
    ...
    if (verbs.length > 0) { const v = verbs[0]; points.push({ title: "Subject + Verb Pattern", pattern: "Subject + " + v.chinese, example: "我" + v.chinese + "。", explanation: `Use "${v.chinese}" (${v.pinyin}) meaning "${v.english}" after a subject to form a basic sentence.` }); }
    if (particles.length > 0) { const p = particles[0]; points.push({ title: "Particle Usage: " + p.chinese, pattern: "Sentence + " + p.chinese, example: "你好" + p.chinese + "？", explanation: `The particle "${p.chinese}" (${p.pinyin}) is used ${p.english}.` }); }
    if (nouns.length >= 2) { points.push({ title: "Noun Phrases", pattern: nouns[0].chinese + " + " + nouns[1].chinese, example: nouns[0].chinese + nouns[1].chinese, explanation: `Combine "${nouns[0].chinese}" (${nouns[0].english}) with "${nouns[1].chinese}" (${nouns[1].english}) to form compound expressions.` }); }
    if (verbs.length >= 2) { points.push({ title: "Verb Series", pattern: "Subject + V1 + V2", example: "我" + verbs[0].chinese + verbs[1].chinese + "。", explanation: `Chinese allows verb serialization: "${verbs[0].chinese}" (${verbs[0].english}) + "${verbs[1].chinese}" (${verbs[1].english}).` }); }
    if (points.length === 0) { points.push({ title: "Basic Word Order", pattern: "Subject + Verb + Object", example: vocab.length > 0 ? "我" + vocab[0].chinese + "。" : "我好。", explanation: "Chinese follows SVO (Subject-Verb-Object) word order, similar to English." }); }
    return points;
  }, [vocab]);
          ...
          <div key={i} className="brutalist-card rounded-xl bg-card p-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 p-3 bg-muted rounded-lg font-mono text-sm"><p className="text-muted-foreground">Pattern: <span className="text-foreground font-bold">{gp.pattern}</span></p></div>
                <div className="mt-2 p-3 bg-primary/5 rounded-lg">
                  <p className="text-lg font-bold">{gp.example}</p>
                  <button onClick={() => speakChinese(gp.example)} className="mt-1 text-xs text-primary flex items-center gap-1 hover:underline"><Volume2 size={12} /> Listen</button>
                </div>
...
const CultureTab = ({ vocab, lessonTitle }: { vocab: VocabWord[]; lessonTitle: string }) => {
  const culturalNotes = useMemo(() => {
    ...
    if (hasFamily) notes.push({ title: "Family Values", content: "Family (家, jiā) is the cornerstone of Chinese society. Respect for elders is deeply ingrained.", emoji: "👨‍👩‍👧‍👦" });
    if (hasFood) notes.push({ title: "Chinese Food Culture", content: "Food holds a sacred place in Chinese culture. Tea (茶, chá) has been consumed for thousands of years.", emoji: "🍵" });
    if (hasTime) notes.push({ title: "Chinese Calendar", content: "China uses both the Gregorian calendar and the traditional lunar calendar.", emoji: "📅" });
    if (notes.length === 0) notes.push({ title: "Chinese Language & Culture", content: `In this lesson about "${lessonTitle}", you'll encounter concepts that reflect Chinese daily life.`, emoji: "🏮" });
    return notes;
  }, [vocab, lessonTitle]);
        ...
        {culturalNotes.map((note, i) => (
          <div key={i} className="brutalist-card rounded-xl bg-card p-6">
            <div className="flex items-start gap-4">
              ...
              <div><h3 className="font-bold text-sm">{note.title}</h3><p className="text-sm text-muted-foreground mt-2 leading-relaxed">{note.content}</p></div>
            </div>
          </div>
        ))}
      </div>
...
const FlashcardsTab = ({ vocab }: { vocab: VocabWord[] }) => {
  ...
  const shuffle = () => { setShuffled([...vocab].sort(() => Math.random() - 0.5)); setIndex(0); setFlipped(false); };
        ...
        <div className="grid grid-cols-2 gap-3 max-w-md mx-auto">
          {q.options.map((opt, i) => (
            <button key={i} onClick={() => handleSelect(opt)} disabled={!!selected}
              className={`p-3 rounded-lg text-sm font-medium transition-colors brutalist-border ${selected ? opt === q.correct ? "bg-green-100 dark:bg-green-900/30 border-green-500 text-green-800 dark:text-green-200" : opt === selected ? "bg-red-100 dark:bg-red-900/30 border-red-500 text-red-800 dark:text-red-200" : "bg-card" : "bg-card hover:bg-muted"}`}>
              {opt}
            </button>
          ))}
        </div>
...
const LessonDetailPage = () => {
  ...
  const { data: allLessons = [], isLoading } = useHskLessons(levelNum);
    ...
    return (
      <div className="max-w-5xl mx-auto">
        <div className="brutalist-card rounded-xl bg-card p-12 text-center">
          ...
          <p className="font-bold text-lg">Lesson not found</p>
          <Link to={`/course/${level}`} className="mt-4 inline-block retro-tag text-primary border-primary hover:bg-primary/10">← Back to course</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <Breadcrumb items={[{ label: "Dashboard", to: "/" }, { label: "Study", to: "/course" }, { label: lvl.toUpperCase(), to: `/course/${level}` }, { label: "LESSON" }, { label: lessonId || "1" }]} />

      {/* Lesson Header */}
      <div className="brutalist-card rounded-xl bg-card p-4 mb-6 flex items-center justify-between">
        <Link to={`/course/${level}`} className="w-9 h-9 rounded-lg bg-muted flex items-center justify-center hover:bg-foreground/10 transition-colors"><ArrowLeft size={18} /></Link>
        ...
        <button onClick={() => toggleComplete(lvl, lessonNum)}
          className={`px-4 py-2 rounded-lg font-mono text-xs font-bold brutalist-border flex items-center gap-1.5 hover:opacity-90 transition-all ${completed ? "bg-green-500 text-white border-green-600" : "bg-secondary text-secondary-foreground"}`}>
      ...
      <div className="flex items-center justify-between mt-4 mb-4">
        {lessonNum > 1 ? (
          <Link to={`/course/${level}/lesson/${lessonNum - 1}`} className="retro-tag text-primary border-primary hover:bg-primary/10 transition-colors flex items-center gap-1"><ChevronLeft size={14} /> Lesson {lessonNum - 1}</Link>
        ) : <div />}
        {lessonNum < maxLesson ? (
          <Link to={`/course/${level}/lesson/${lessonNum + 1}`} className="retro-tag text-primary border-primary hover:bg-primary/10 transition-colors flex items-center gap-1">Lesson {lessonNum + 1} <ChevronRight size={14} /></Link>
        ) : <div />}
      </div>
    </div>
  );
